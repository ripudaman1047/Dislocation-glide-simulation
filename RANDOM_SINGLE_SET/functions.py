...
def get_center(p1, p2, radius):     
    '''
        This functions takes two obstacle objects and a radius to output a circle's center coordinates
        with given radius, on whose circumfrence the two given points would lie.
...
def circle_roll(p1, dis, Area, rad):
    '''
        ...
        Area : list (consistin all the points around p1 within 2*rad distance)
        rad: float
...
def swept_ar(p1, rad, tree):
    '''
        ...
        arr : list
    '''
    indices = tree.query_ball_point([p1.x, p1.y], 2*rad) 
