...
def plot(dis, disLine, tau, theta1, theta2, theta3):
    '''
        ...
        -color1 & color2 (colors of the obstacles points in set1, set2 & set3)
    ...
    array3 = np.array([[obs.x, obs.y] for obs in obstacle.array if obs.brAngle==theta3])
    ...
    plt.scatter(array3[:,0], array3[:,1], s=size, c=color3, edgecolor='black', linewidths=1)
    ...
    array4 = np.array([[obs.x, obs.y] for obs in dis if obs.brAngle==theta1])
    array5 = np.array([[obs.x, obs.y] for obs in dis if obs.brAngle==theta2])
    array6 = np.array([[obs.x, obs.y] for obs in dis if obs.brAngle==theta3])
    
    if len(array4)!=0:
        plt.scatter(array4[:,0], array4[:,1], s=size, c=color1, edgecolor=discolor, linewidths=1)
    if len(array5)!=0:
        plt.scatter(array5[:,0], array5[:,1], s=size, c=color2, edgecolor=discolor, linewidths=1)
    if len(array6)!=0:
        plt.scatter(array6[:,0], array6[:,1], s=size, c=color3, edgecolor=discolor, linewidths=1) 
