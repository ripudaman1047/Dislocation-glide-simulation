...
def main():
    ...
    f2 = get_float("Fraction of set-2 (0 to 1): ", errstr, Min=0, Max=1)
    while f1+f2>=1:
        print("Invalid response! Select a correct value")
        f2 = get_float("Fraction of set-2 (0 to 1): ", errstr, Min=0, Max=1)
    ...
    theta3 = get_float("Breaking angle for set-3: ", errstr, Min=0, Max=180)

    size_set1 = round(f1*arrsize)
    size_set2 = round(f2*arrsize)
    size_set3 = arrsize-size_set1-size_set2

    br1 = [theta1]*size_set1
    br2 = [theta2]*size_set2
    br3 = [theta3]*size_set3

    dist = np.array(br1+br2+br3)
    random.shuffle(dist)
    ...
    while not dis == None:
        ...
        if imageflag==1:
            ...
            plot(dis, disLine, tau, theta1, theta2, theta3)
    ...
    with open('results.txt', 'w') as f:
        ...
        f.write("Breaking angle of set-3= {0}\n".format(theta3))
        f.write("Fraction of set-1= {0}\n".format(f1))
        f.write("Fraction of set-2= {0}\n".format(f2))
        f.write("Fraction of set-3= {0}\n".format(round(1-f1-f2,3)))
        f.write("CRSS= {0}\n".format(CRSS))
    ...
    obs_name = str(arrsize)+"-"+str(theta1)+"-"+str(theta2)+"-"+str(theta3)+".csv"
